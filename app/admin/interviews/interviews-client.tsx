...
export function InterviewsClient({ interviews: init, applications }: { interviews: any[]; applications: any[] }) {
        ...
        <form onSubmit={handleCreate} className="rounded-xl border border-border bg-card p-6 space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <Input value={form.interviewerName} onChange={(e) => setForm({ ...form, interviewerName: e.target.value })} placeholder="e.g. Rajesh Kumar" />
            </div>
            <div>
              <Label>Mode</Label>
        ...
        <form onSubmit={handleEdit} className="rounded-xl border-2 border-primary/30 bg-card p-6 space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div>
              ...
              <Input value={editForm.feedback} onChange={(e) => setEditForm({ ...editForm, feedback: e.target.value })} placeholder="Interview feedback..." />
            </div>
          </div>
        ...
        {init.map((i: any) => (
          <div key={i.id} className="rounded-xl bg-card shadow-sm p-5 flex items-center gap-4">
            ...
            <Button variant="ghost" size="icon" onClick={() => {
              setEditId(i.id);
              // Convert scheduledAt to datetime-local format (YYYY-MM-DDTHH:MM)
