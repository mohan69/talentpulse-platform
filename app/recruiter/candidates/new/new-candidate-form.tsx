...
export function NewCandidateForm() {
  ...
  async function save() {
    ...
    try {
      ...
      router.push(`/recruiter/candidates/${data.id}`);
  ...
  return (<div className="grid lg:grid-cols-2 gap-6">
    ...
    <div className="rounded-xl bg-card shadow-sm p-5 space-y-3"><h3 className="font-semibold mb-2">Candidate Details</h3>
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="col-span-2"><Label>Skills (comma-separated)</Label><Input value={(form.skills ?? []).join(", ")} onChange={(e) => setForm({ ...form, skills: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) })} /></div>
        <div className="col-span-2"><Label>Summary</Label><Textarea value={form.aiSummary} onChange={(e) => setForm({ ...form, summary: e.target.value })} rows={4} /></div>
      </div>
      <Button onClick={save} disabled={saving} className="w-full">{saving ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Saving</> : "Save Candidate"}</Button>
    </div>
  </div>);
}
