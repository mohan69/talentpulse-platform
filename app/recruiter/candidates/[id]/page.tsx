        ...
        <div className="md:col-span-2 space-y-6">
          <div className="rounded-xl bg-card shadow-sm p-5">
            ...
            <div className="grid sm:grid-cols-2 gap-3 text-sm">
              ...
              <div>Expected: {c.expectedCtc ? formatCurrency(c.expectedCtc) : "-"}</div>
            </div>
            <div className="mt-4">
              <div className="text-xs font-semibold mb-2">Skills</div>
              <div className="flex flex-wrap gap-1.5">{(c.skills ?? []).map((s) => <span key={s} className="px-2 py-0.5 text-xs rounded bg-primary/10 text-primary">{s}</span>)}</div>
            </div>
          ...
          {c.applications[0] && <AIScreeningPanel applicationId={c.applications[0].id} initialScore={c.applications[0].matchScore} initialReport={c.applications[0].aiReport as any} />}
          <div className="rounded-xl bg-card shadow-sm p-5">
            ...
            <div className="space-y-3">
              {c.projects.map((p) => (
                <div key={p.id} className="p-3 rounded-lg bg-muted/40">
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">{(p.skillsUsed ?? []).map((t) => <span key={t} className="px-1.5 py-0.5 text-xs rounded bg-slate-100">{t}</span>)}</div>
                </div>
              ))}
            ...
            <div className="space-y-2">
              {c.applications.map((a) => (
                <div key={a.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/40">
                  <div><div className="font-medium">{a.job.title}</div><div className="text-xs text-muted-foreground">{a.job.client?.name}</div></div>
