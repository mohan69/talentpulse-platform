...
export function RecruiterTemplatesClient({ templates }: { templates: any[] }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button onClick={() => { setShowAdd(!showAdd); setEditId(null); }}>{showAdd ? <><X className="h-4 w-4 mr-2" />Cancel</> : <><Plus className="h-4 w-4 mr-2" />New Template</>}</Button>
      </div>
              ...
              <>
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => { setEditId(t.id); setEditForm({ subject: t.subject, body: t.body, category: t.category }); setShowAdd(false); }}><Pencil className="h-3.5 w-3.5" /></Button>
                  </div>
                </div>
